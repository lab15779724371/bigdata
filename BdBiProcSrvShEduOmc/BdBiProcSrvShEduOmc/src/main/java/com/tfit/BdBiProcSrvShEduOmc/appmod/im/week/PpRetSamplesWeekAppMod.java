...
//留样周报表
public class PpRetSamplesWeekAppMod {
	private static final Logger logger = LogManager.getLogger(PpRetSamplesWeekAppMod.class.getName());
	...
	// 项目点排菜详情列表函数（方案2）
	PpRetSamplesWeekOutDto ppDishWeekFromHive(String[] dates, 
		...
		List<AppTEduReserveTotalW> appTEduPlatoonTotalWList = dbHiveRetSamplesService.getAppTEduReserveTotalWList(listYearMonth, startDate, 
				endDateAddOne, inputObj, statPage, endPage);
		appTEduPlatoonTotalWList.removeAll(Collections.singleton(null));
		
		Integer pageTotalTemp = dbHiveRetSamplesService.getAppTEduReserveTotalWListCount(listYearMonth, startDate, endDateAddOne, inputObj);
		
		if(pageTotalTemp!=null) {
			pageTotal = pageTotalTemp;
		}
		
		List<PpRetSamplesWeekDto> ppDishWeekDto = new ArrayList<>();
		for(AppTEduReserveTotalW obj : appTEduPlatoonTotalWList) {
			PpRetSamplesWeekDto pdd = new PpRetSamplesWeekDto();
			try {
				BeanUtils.copyProperties(pdd, obj);
				
				pdd.setUseDatePeriod((obj.getStartUseDate()==null?"":obj.getStartUseDate().replaceAll("-", "/")) + "-" + (obj.getEndUseDate()==null?"":obj.getEndUseDate().replaceAll("-", "/")));
		...
		PpRetSamplesWeekOutDto pddDto = new PpRetSamplesWeekOutDto();
		// 设置返回数据
		...
		//设置数据
		pddDto.setPpRetSamplesWeekList(ppDishWeekDto);
		//消息ID
	...
	//项目点排菜详情列表模型函数
	public PpRetSamplesWeekOutDto appModFunc(String token,  
			AppTEduReserveTotalW inputObj,
		...
		if(inputObj == null) {
			inputObj = new AppTEduReserveTotalW();
		...
		if (isRealData) { // 真实数据
			// 日期
			String[] dates = null;
			if (inputObj.getStartUseDate() == null) { // 按照当天日期获取数据
				dates = new String[1];
				dates[0] = BCDTimeUtil.convertNormalDate(null);
				inputObj.setStartUseDate(BCDTimeUtil.convertNormalDate(null));
			} else { // 按照开始日期和结束日期获取数据
				DateTime startDt = BCDTimeUtil.convertDateStrToDate(inputObj.getStartUseDate());
				DateTime endDt = BCDTimeUtil.convertDateStrToDate(inputObj.getEndUseDate());
			...
			if (inputObj.getArea() != null && inputObj.getPrefCity() == null && inputObj.getProvince() != null) {    // 按区域，省或直辖市处理
				...
				if (bfind) {
					...
					//if(days >= 2) {
						return ppDishWeekFromHive(dates, inputObj, dbHiveRetSamplesService);
					//}
				}
		...
		List<PpRetSamplesWeekDto> dishList = new ArrayList<>();
		PpRetSamplesWeekOutDto pddDto = new PpRetSamplesWeekOutDto();
		// 设置返回数据
		pddDto.setTime(BCDTimeUtil.convertNormalFrom(null));
		// 分页
		...
		pddDto.setPageInfo(pageInfo);
		//设置数据
		pddDto.setPpRetSamplesWeekList(dishList);
		//消息ID
		pddDto.setMsgId(AppModConfig.msgId);
		AppModConfig.msgId++;
		// 消息id小于0判断

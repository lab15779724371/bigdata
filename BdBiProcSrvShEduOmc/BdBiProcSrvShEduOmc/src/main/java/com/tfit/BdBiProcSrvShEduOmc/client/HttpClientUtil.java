...
public class HttpClientUtil {  
    ...
    public static String doGet(String url, String charset) throws Exception {  
        ...
        InternalCallerContext Icc = new InternalCallerContext();
  
        if (null == url || !url.startsWith("http")) {  
            throw new Exception("请求地址格式不对");  
        }  
        // 设置header部分
        method.addRequestHeader("InternalCallerContext", Icc.toBase64());
        // 设置请求的编码方式  
        if (null != charset) {  
...
//        }  
    ...
    public static void getRequestTest() {      	  
        String url = "http://dev.tfcsp.com/api/v1.0/cateringservice/catering_provider/employee_dispatch/get_info?employee_entity_type_id=649414543&employee_entity_id=94394086680690688";  
        try {  
        	logger.info("输入url：" + url);

	...
	public String appModFunc(HttpServletRequest request, Db1Service db1Service, Db2Service db2Service) {
		...
		try {
			...
			if (verTokenFlag) {
				...
				if(id !=null){
					AppCommonDao detailDao = db2Service.getCheckTask(id);
					// 获取列表数据
					LinkedHashMap<String, Object> commonMap = detailDao.getCommonMap();
					appCommonExternalModulesDto.setData(commonMap);
				}else {
					appCommonExternalModulesDto.setResCode(IOTRspType.System_ERR.getCode().toString());
					appCommonExternalModulesDto.setResMsg("任务id不能为空");
				}

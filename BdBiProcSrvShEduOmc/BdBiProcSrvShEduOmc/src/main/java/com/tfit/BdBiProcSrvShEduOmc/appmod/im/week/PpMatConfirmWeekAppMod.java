...
import com.tfit.BdBiProcSrvShEduOmc.dto.im.week.PpMatConfirmWeekDto;
...
//用料确认周报表
public class PpMatConfirmWeekAppMod {
	private static final Logger logger = LogManager.getLogger(PpMatConfirmWeekAppMod.class.getName());
	...
	// 项目点排菜详情列表函数（方案2）
	PpMatConfirmWeekOutDto ppMatConfirmWeekFromHive(String[] dates, 
		...
		List<AppTEduMaterialTotalWObj> appTEduPlatoonTotalWList = dbHiveMatService.getAppTEduMaterialTotalWObjList(listYearMonth, startDate, endDateAddOne, 
				inputObj, statPage, endPage);
		appTEduPlatoonTotalWList.removeAll(Collections.singleton(null));
		
		Integer pageTotalTemp = dbHiveMatService.getAppTEduMaterialTotalWObjListCount(listYearMonth, startDate, endDateAddOne, inputObj);
		
		if(pageTotalTemp!=null) {
			pageTotal = pageTotalTemp;
		}
		
		List<PpMatConfirmWeekDto> ppMatConfirmWeekDto = new ArrayList<>();
		for(AppTEduMaterialTotalWObj obj : appTEduPlatoonTotalWList) {
			PpMatConfirmWeekDto pdd = new PpMatConfirmWeekDto();
			try {
		...
		PpMatConfirmWeekOutDto pddDto = new PpMatConfirmWeekOutDto();
		// 设置返回数据
		...
		//设置数据
		pddDto.setPpMatConfirmWeekList(ppMatConfirmWeekDto);
		//消息ID
	...
	//项目点排菜详情列表模型函数
	public PpMatConfirmWeekOutDto appModFunc(String token,  
			AppTEduMaterialTotalWObj inputObj,
		...
		if(inputObj == null) {
			inputObj = new AppTEduMaterialTotalWObj();
		...
		if (isRealData) { // 真实数据
			...
			if (inputObj.getArea() != null && inputObj.getPrefCity() == null && inputObj.getProvince() != null) {    // 按区域，省或直辖市处理
				...
				if (bfind) {
					...
					//if(days >= 2) {
						return ppMatConfirmWeekFromHive(dates, inputObj, dbHiveMatService);
					//}
				}
		...
		List<PpMatConfirmWeekDto> dishList = new ArrayList<>();
		PpMatConfirmWeekOutDto pddDto = new PpMatConfirmWeekOutDto();
		// 设置返回数据
		pddDto.setTime(BCDTimeUtil.convertNormalFrom(null));
		// 分页
		...
		pddDto.setPageInfo(pageInfo);
		//设置数据
		pddDto.setPpMatConfirmWeekList(dishList);
		//消息ID
		pddDto.setMsgId(AppModConfig.msgId);
		AppModConfig.msgId++;
		// 消息id小于0判断

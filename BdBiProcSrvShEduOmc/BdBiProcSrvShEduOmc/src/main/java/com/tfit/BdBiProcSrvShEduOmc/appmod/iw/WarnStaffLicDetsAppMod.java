...
public class WarnStaffLicDetsAppMod {
	...
	// 证照预警人员证件详情列表函数
	WarnStaffLicDetsDTO warnStaffLicDetsTwo(String departmentId,String distIdorSCName, String startDate, String endDate,
			 DbHiveWarnService dbHiveWarnService,
			 String rmcName, int licType, int licStatus, int licAuditStatus, String licAuditStatuss, String fullName, 
			 String startElimDate, String endElimDate, String startValidDate, String endValidDate, 
			 String licNo, String schName,Integer target,
			 String departmentIds,int schType,int schProp) {
		WarnStaffLicDetsDTO wstldDto = new WarnStaffLicDetsDTO();
		List<WarnStaffLicDets> warnStaffLicDets = new ArrayList<>();
		...
		try {
			...
			warnCommonLicDets = dbHiveWarnService.getWarnLicDetsList(1,3,listYearMonth, startDate,
			...
			for(WarnCommonLicDets warnCommon :warnCommonLicDets) {
				WarnStaffLicDets warn = new WarnStaffLicDets();
				BeanUtils.copyProperties(warn, warnCommon);
				warnStaffLicDets.add(warn);
			}
			
			Integer pageTotalTemp = dbHiveWarnService.getWarnLicDetsCount(1,3,listYearMonth, startDate, 
					endDateAddOne, null, schName, schType, licType, licStatus, 
					licAuditStatus,licAuditStatussList, startElimDate, endElimDate, startValidDate, 
					endValidDate, schProp, licNo,rmcName,fullName,target,departmentId,departmentIdsList,null,distIdorSCName,null,null);
	...
	// 证照预警人员证件详情列表模型函数
	public WarnStaffLicDetsDTO appModFunc(String token, String startWarnDate, String endWarnDate, String distName,
			...
			String fullName, String startElimDate, String endElimDate, String startValidDate, String endValidDate, 
			String licNo, String schName,
		...
		if(isRealData) {       //真实数据
			...
			if (distName != null && prefCity == null && province != null) {    // 按区域，省或直辖市处理
				...
				if (bfind) {
					...
					// 证照预警人员证件详情列表函数
					if(methodIndex == 1 ) {
						wstldDto = warnStaffLicDets(distIdorSCName, dates, tedList, db1Service, saasService, rmcName, curLicType, 
								curLicStatus, curLicAuditStatus, fullName, startElimDate, endElimDate, startValidDate, 
								endValidDate, licNo, schName);		
					}else if (methodIndex == 2) {
						Integer target = CommonUtil.getTarget(token, db1Service, db2Service);
						wstldDto = warnStaffLicDetsTwo(departmentId,distIdorSCName, startWarnDate, endWarnDate, dbHiveWarnService, rmcName, curLicType, 
								curLicStatus, curLicAuditStatus,licAuditStatuss, fullName, startElimDate, endElimDate, startValidDate, 
								endValidDate, licNo, schName,target,
								departmentIds,curSchType,curSchProp);		
					}
				}
				...
				if (bfind) {
					...
					// 证照预警人员证件详情列表函数
					if(methodIndex == 1 ) {
						wstldDto = warnStaffLicDets(distIdorSCName, dates, tedList, db1Service, saasService, rmcName, 
								curLicType, curLicStatus, curLicAuditStatus, fullName, startElimDate, endElimDate, 
								startValidDate, endValidDate, licNo, schName);
					}else if (methodIndex == 2) {
						Integer target = CommonUtil.getTarget(token, db1Service, db2Service);
						wstldDto = warnStaffLicDetsTwo(departmentId,distIdorSCName, startWarnDate, endWarnDate, dbHiveWarnService, rmcName, curLicType, 
								curLicStatus, curLicAuditStatus,licAuditStatuss, fullName, startElimDate, endElimDate, startValidDate, 

...
public class DeleteMyaffairMod {
	// 是否为真实数据标识
	...
	public String appModFunc(HttpServletRequest request, Db1Service db1Service, Db2Service db2Service) {
		...
		try {
			...
			if (verTokenFlag) {
				...
				if(request.getParameter("affairId") !=null) {
					boolean flag=db2Service.getDeleteMyaffair(id);
					if(flag==false) {
						appCommonExternalModulesDto.setResCode(IOTRspType.System_ERR.getCode().toString());
						appCommonExternalModulesDto.setResMsg("删除失败");
					}
				}else {
					appCommonExternalModulesDto.setResCode(IOTRspType.System_ERR.getCode().toString());

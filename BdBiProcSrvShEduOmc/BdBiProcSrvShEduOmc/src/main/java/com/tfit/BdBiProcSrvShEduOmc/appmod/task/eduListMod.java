	...
	public String appModFunc(HttpServletRequest request, Db1Service db1Service, Db2Service db2Service) {
		...
		try {
			...
			if (verTokenFlag) {
				...
				String org_name=userINfo.getOrgName();
				sourceDao = db2Service.geteduList();
				// 获取列表数据
			for (int i = 0; i < sourceDao.size(); i++) {
				LinkedHashMap<String, Object> commonMap = sourceDao.get(i).getCommonMap();
				if("市教委".equals(org_name)) {
					dataList.add(commonMap);
				}else {
					if (org_name.equals(commonMap.get("name"))) {
						dataList.add(commonMap);
					}
				}
				}
...
////						dataList.add(commonMap);
////					}else {
////						if(!"市教委".equals(commonMap.get("name"))) {
////							dataList.add(commonMap);
////						}
//					}
//				}

...
import com.tfit.BdBiProcSrvShEduOmc.dto.iw.WarnStaffLics;
...
public class WarnStaffLicsAppMod {
	...
	private WarnStaffLicsDTO warnStaffLicsTwo(String distIdorSCName, String startDate, String endDate, List<TEduDistrictDo> tedList,Integer target,
			DbHiveWarnService dbHiveWarnService) {
		WarnStaffLicsDTO wslDto = new WarnStaffLicsDTO();
		List<WarnStaffLics> warnStaffLics = new ArrayList<>();
		WarnStaffLics wsl = null;
		...
		warnCommonLicList = dbHiveWarnService.getWarnLicList(3, listYearMonth, startDate, endDateAddOne, distIdorSCName,target);
		
		/**
		 * 2.合并数据(按区合并)
		 */
		Map<String, WarnStaffLics> warnTotalMap = new HashMap<String, WarnStaffLics>();
		for(WarnCommonLics warnCommonLics : warnCommonLicList) {
			WarnStaffLics warnCommonLicsTemp =warnTotalMap.get(warnCommonLics.getDistName());
			if(warnCommonLicsTemp == null) {
				warnCommonLicsTemp = new WarnStaffLics();
		...
		// 设置数据
		wslDto.setWarnStaffLics(pageBean.getCurPageData());
		// 消息ID
	...
	private WarnStaffLicsDTO warnStaffLicsThree(String departmentId,String distIdorSCName, String distNames,String startDate, String endDate, List<TEduDistrictDo> tedList,Integer target,
			DbHiveWarnService dbHiveWarnService) {
		WarnStaffLicsDTO wslDto = new WarnStaffLicsDTO();
		List<WarnStaffLics> warnStaffLics = new ArrayList<>();
		WarnStaffLics wsl = null;
		...
		warnCommonList = dbHiveWarnService.getWarnList(1, 3, target, listYearMonth, startDate, endDateAddOne,
	...
	private WarnStaffLicsDTO warnStaffLicsByDepartment(String departmentId,String departmentIds,String startDate, String endDate, List<DepartmentObj> departmntList,Integer target,
			DbHiveWarnService dbHiveWarnService) {
		WarnStaffLicsDTO wslDto = new WarnStaffLicsDTO();
		List<WarnStaffLics> warnStaffLics = new ArrayList<>();
		WarnStaffLics wsl = null;
		...
		Map<String, WarnStaffLics> warnTotalMap = new HashMap<String, WarnStaffLics>();
		for(WarnCommon warnCommonLics : warnCommonList) {
			WarnStaffLics warnCommonLicsTemp =warnTotalMap.get(warnCommonLics.getDepartmentId());
			if(warnCommonLicsTemp == null) {
				warnCommonLicsTemp = new WarnStaffLics();
	...
	// 证照预警人员证件列表模型函数
	public WarnStaffLicsDTO appModFunc(String token, String startWarnDate, String endWarnDate, String distName, String prefCity, 
			...
			if (distName != null && prefCity == null && province != null) { // 按区域，省或直辖市处理
				...
				if (bfind) {
					...
					if(methodIndex == 1) {
						...
						wstlDto = warnStaffLicsTwo(distIdorSCName, startWarnDate,endWarnDate, tedList,target, dbHiveWarnService);
					}else if (methodIndex == 3) {
						Integer target = CommonUtil.getTarget(token, db1Service, db2Service);
						
						if("1".equals(schSelMode)) {  //按所在地
							wstlDto = warnStaffLicsThree(deparmentId,distIdorSCName,distNames, startWarnDate,startWarnDate, tedList,target, dbHiveWarnService);
							...
							wstlDto = warnStaffLicsByDepartment(deparmentId, departmentIds, startWarnDate, startWarnDate, deparmentList, target, dbHiveWarnService);
						}
					}
				}
				...
				if (bfind) {
					...
					if(methodIndex == 1) {
						...
						Integer target = CommonUtil.getTarget(token, db1Service, db2Service);
						if("1".equals(schSelMode)) {  //按所在地
							wstlDto = warnStaffLicsThree(deparmentId,distIdorSCName,distNames, startWarnDate,endWarnDate, tedList,target, dbHiveWarnService);
						}else if("2".equals(schSelMode)) {  //按管理部门
							List<Object> departmentIdsList=CommonUtil.changeStringToList(departmentIds);
							List<DepartmentObj> deparmentList =  db1Service.getDepartmentObjList(new DepartmentObj(),departmentIdsList, -1, -1);
							wstlDto = warnStaffLicsByDepartment(deparmentId, departmentIds, startWarnDate, endWarnDate, deparmentList, target, dbHiveWarnService);
						}
					}
				}

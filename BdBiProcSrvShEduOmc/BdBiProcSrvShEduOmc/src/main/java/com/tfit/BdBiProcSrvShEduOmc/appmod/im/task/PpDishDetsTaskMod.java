...
//项目点排菜详情列表应用模型
public class PpDishDetsTaskMod {
	private static final Logger logger = LogManager.getLogger(PpDishDetsTaskMod.class.getName());
	...
	boolean ppDishDetsFromHive(int warnLevel,Db1Service db1Service,Db2Service db2Service,
		...
		logger.info("=====================================排菜"+AppModConfig.warnLevelIdToNameMap.get(warnLevel)+"预警"+"开始");
		...
		if(warnCommonLicDets != null && warnCommonLicDets.size()>0) {
			...
			for(WarnCommonLicDets warnCommon :warnCommonLicDets) {
				...
				DishEmailDto pdd = new DishEmailDto(warnCommon.getDepartmentId(),warnCommon.getSchTypeId()==null?1000:warnCommon.getSchTypeId(),warnCommon.getSchType(),warnCommon.getSchName(),warnCommon.getDinnerDate(),closingDate);
			...
			List<AppCommonDao> checkWarnSetting  = db2Service.getCheckWarnSetting(1, warnLevel);
			...
			if (null != warnLevelBody){
				if (warnLevelBody.getScheduledStatus() !=null && warnLevelBody.getScheduledStatus() == 1){
					...
					if(pushReceiverMsg!=null &&pushReceiverMsg.size()>0) {
						//****年**月**日排菜/验收信息未上报提示/提醒/预警/督办/追责名单
					    String mailTitle = closingDate.substring(0,closingDate.indexOf(" "))+"排菜信息未上报"+AppModConfig.warnLevelIdToNameMap.get(warnLevel)+"名单";
					    String newMailTitle = "";
						...
						for(PushReceiverInfo pushReceiverInfo : pushReceiverMsg) {
							 ...
							 */
							if(emailStatus ==1) {
								boolean flag = SendMailCommon.sendNoDishMail(db2Service, newMailTitle,tebuDo.getEmail(),closingDate,dishDate,departmentName,newDataMap,warnAccount);
								if(!flag) {
							 ...
							 */
							if(tebuDo !=null) {
								new WarnPushTaskMod().insertPushData(db2Service,warnAccount, 1, warnLevel, newMailTitle, pushDate, tebuDo.getUserAccount(), closingDate, dishDate, departmentName, newDataMap);
							}
						}
			...
			logger.info("排菜"+AppModConfig.warnLevelIdToNameMap.get(warnLevel)+"预警 "+"轮询第"+pollTimes+"次================================================");
			//轮询10次，仍然未取到数据，停止服务
			if(pollTimes == 5) {
				logger.info("排菜"+AppModConfig.warnLevelIdToNameMap.get(warnLevel)+"预警 "+"轮询第"+pollTimes+"次,停止轮询================================================");
				return false;
			}
		...
		logger.info("=====================================排菜"+AppModConfig.warnLevelIdToNameMap.get(warnLevel)+"预警"+"结束"+pollTimes);
		return result;
	}	

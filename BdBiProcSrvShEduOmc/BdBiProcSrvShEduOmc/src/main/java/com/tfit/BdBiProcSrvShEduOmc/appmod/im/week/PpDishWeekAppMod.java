...
//排菜周报表
public class PpDishWeekAppMod {
	private static final Logger logger = LogManager.getLogger(PpDishWeekAppMod.class.getName());
	...
	// 项目点排菜详情列表函数（方案2）
	PpDishWeekOutDto ppDishWeekFromHive(String[] dates, 
		...
		List<AppTEduPlatoonTotalWObj> appTEduPlatoonTotalWList = dbHiveDishService.getAppTEduPlatoonTotalWObjList(listYearMonth, startDate, endDateAddOne, 
				inputObj, statPage, endPage);
		appTEduPlatoonTotalWList.removeAll(Collections.singleton(null));
		
		Integer pageTotalTemp = dbHiveDishService.getAppTEduPlatoonTotalWObjListCount(listYearMonth, startDate, endDateAddOne, inputObj);
		
		if(pageTotalTemp!=null) {
			pageTotal = pageTotalTemp;
		}
		
		List<PpDishWeekDto> ppDishWeekDto = new ArrayList<>();
		for(AppTEduPlatoonTotalWObj obj : appTEduPlatoonTotalWList) {
			PpDishWeekDto pdd = new PpDishWeekDto();
			try {
		...
		PpDishWeekOutDto pddDto = new PpDishWeekOutDto();
		// 设置返回数据
		...
		//设置数据
		pddDto.setPpDishWeekList(ppDishWeekDto);
		//消息ID
	...
	//项目点排菜详情列表模型函数
	public PpDishWeekOutDto appModFunc(String token,  
			AppTEduPlatoonTotalWObj inputObj,
		...
		if(inputObj == null) {
			inputObj = new AppTEduPlatoonTotalWObj();
		...
		if (isRealData) { // 真实数据
			...
			if (inputObj.getArea() != null && inputObj.getPrefCity() == null && inputObj.getProvince() != null) {    // 按区域，省或直辖市处理
				...
				if (bfind) {
					...
					//if(days >= 2) {
						return ppDishWeekFromHive(dates, inputObj, dbHiveDishService);
					//}
				}
		...
		List<PpDishWeekDto> dishList = new ArrayList<>();
		PpDishWeekOutDto pddDto = new PpDishWeekOutDto();
		// 设置返回数据
		pddDto.setTime(BCDTimeUtil.convertNormalFrom(null));
		// 分页
		...
		pddDto.setPageInfo(pageInfo);
		//设置数据
		pddDto.setPpDishWeekList(dishList);
		//消息ID
		pddDto.setMsgId(AppModConfig.msgId);
		AppModConfig.msgId++;
		// 消息id小于0判断

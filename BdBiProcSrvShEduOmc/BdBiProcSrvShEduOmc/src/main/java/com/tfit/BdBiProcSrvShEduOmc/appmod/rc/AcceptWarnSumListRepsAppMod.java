...
//排菜周报表
public class AcceptWarnSumListRepsAppMod {
	private static final Logger logger = LogManager.getLogger(AcceptWarnSumListRepsAppMod.class.getName());
	...
	DishWarnSumListRepsOutDto ppDishWeekFromHive(String[] dates, 
		...
		List<AppTEduNoLedgerCollectD> appTEduNoLedgerCollectDList = dbHiveWarnService.getAppTEduNoLedgerCollectDList(listYearMonth, startDate, endDateAddOne, 
				inputObj, statPage, endPage);
		
		appTEduNoLedgerCollectDList.removeAll(Collections.singleton(null));
		
		Integer pageTotalTemp = dbHiveWarnService.getAppTEduNoLedgerCollectDListCount(listYearMonth, startDate, endDateAddOne, inputObj);
		...
		statPage++;
		for(AppTEduNoLedgerCollectD warnCommon :appTEduNoLedgerCollectDList) {
